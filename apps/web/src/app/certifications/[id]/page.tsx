...
export default async function CertificationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const definitions = new Map<string, string>(
    result.protocol.definition.criteria.map((criterion: { id: string; title: string }) => [criterion.id, criterion.title]),
  );
    ...
    <article>
      <div className="btn-row">
        <Badge tone={active && result.outcome === "certified" ? "success" : "neutral"}>
          {presentation?.label ?? `${result.certifier.name}: ${result.outcome.replaceAll("-", " ")}`}
        </Badge>
      ...
      <div className="grid layout-2">
        <div>
          <Card title="Criterion assessment">
            {criteria.map((criterion) => (
              <section key={criterion.criterionId}>
                <h2>{criterion.criterionId.toUpperCase()} — {definitions.get(criterion.criterionId)}</h2>
          ...
          <Card title="Limitations">
            {result.limitations.length ? <ul>{result.limitations.map((item: string) => <li key={item}>{item}</li>)}</ul> : <p>None recorded.</p>}
          </Card>
        </div>
        <aside>
          <Card title="Exact assessment identity">
            <DefinitionList items={[
              ...
              { term: "Protocol", value: `${result.protocol.title} · ${result.protocol.seriesKey} v${result.protocol.version}` },
              { term: "PublicationVersion", value: <span className="mono">{result.publicationVersionId}</span> },
              ...
              { term: "Packet SHA-256", value: <span className="mono">{result.input.packetSha256}</span> },
              { term: "Content coverage", value: result.input.completeness.content.coverage },
              { term: "Returned content", value: String(result.input.completeness.content.returnedDocuments) },
            ]} />
          </Card>
          <Card title="Accountability and provenance">
            <DefinitionList items={[
              ...
              { term: "Provider / model", value: result.execution ? `${result.execution.provider} / ${result.execution.model}` : "not recorded" },
              { term: "Prompt", value: result.execution?.promptVersion ?? "not recorded" },
              { term: "Execution packet hash", value: result.execution?.packetHash ?? "not recorded" },
            ]} />
          </Card>
          <Card title="Lifecycle">
            <ol>{result.lifecycle.map((event: { kind: string; createdAt: string; reason: string | null }) => <li key={`${event.kind}:${event.createdAt}`}><strong>{event.kind}</strong> · {event.createdAt}{event.reason ? ` — ${event.reason}` : ""}</li>)}</ol>
          </Card>
          <Card title="Machine-readable API"><a href={`/api/certification-results/${result.id}`}>JSON for this exact result</a></Card>
        </aside>
      </div>
    </article>
  );
...
async function load(id: string) {
  try { return await getPublicCertificationResult(id); }
  catch (error) {
...
function evidenceHref(publicationVersionId: string, reference: { type: string; id?: string; url?: string }) {
