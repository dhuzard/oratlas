...
export default async function PublicationVersionPage({
  ...
  const completeness = parseJsonColumn<{
    ...
    truncated: boolean;
  }>(version.contentCompletenessJson, {
  ...
  return (
    <article>
      ...
      {demo ? (
        <p className="lead">
          A synthetic fixture for demonstrating the certification platform. It is not a real article
      ...
      <div className="grid layout-2">
        <div>
          <Card title="Attributed certifications">
                  ...
                  return (
                    <li key={certification.id}>
                      <a href={`/certifications/${certification.id}`}>
                        <strong>
                          {presentation?.label ??
                            `${certification.certifier.name}: ${certification.outcome.replaceAll("-", " ")}`}
                        </strong>
                      </a>
                      <br />
            ...
            <p className="muted">
              Results from different certifiers remain separate attributed assertions; disagreement
              is not collapsed into a consensus score.
            </p>
          </Card>
          {readiness ? (
