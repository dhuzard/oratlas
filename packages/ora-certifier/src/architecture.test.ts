...
describe("ORA certifier architecture", () => {
  it("uses the shared public API client and contains no database shortcut", () => {
    const source = readFileSync(resolve(process.cwd(), "packages/ora-certifier/src/index.ts"), "utf8");
    ...
    expect(source).not.toMatch(/@prisma|PrismaClient|@oratlas\/db|\.certificationResult\.|submitCertificationResult/);
  });
