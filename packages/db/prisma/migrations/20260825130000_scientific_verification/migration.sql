...
BEGIN
  IF TG_TABLE_NAME = 'VerificationArtifact' AND NOT EXISTS (
    SELECT 1 FROM "VerificationRun" r WHERE r."id" = NEW."verificationRunId"
  ...
  END IF;
  IF TG_TABLE_NAME = 'VerificationFindingArtifact' AND NOT EXISTS (
    SELECT 1 FROM "VerificationFinding" f JOIN "VerificationArtifact" a
