...
export interface CslItem {
  ...
  keyword?: string;
  note?: string;
  "oratlas-platform-version": string;
}
...
export function bibtex(input: VersionExportInput): string {
  ...
  fields.push(["oratlasplatformversion", bibtexEscape(input.platformVersion)]);
  if (input.versionDoi && !input.isExample) {
    fields.push(["doi", bibtexEscape(input.versionDoi)]);
  }
  const notes = [
    `Source repository: ${input.repositoryUrl} at commit ${input.commitSha}.`,
    `Generated by Open Review Atlas platform ${input.platformVersion}.`,
  ];
  if (input.isExample && (input.versionDoi || input.conceptDoi)) {
