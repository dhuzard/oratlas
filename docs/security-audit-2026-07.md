...
| Formal challenge filing, transition, response, and both moderation mutations share the browser-mutation boundary                        | Verified              | Five-route origin/auth/rate/body matrix in `apps/web/src/app/api/challenge-mutation-routes.test.ts`; role and illegal-transition cases in `challenges.integration.test.ts`                            |
...
| Challenge response and moderation writes use revision CAS and commit immutable records, lifecycle transitions, and audit events atomically              | Verified   | `apps/web/src/lib/challenges.ts`; response/moderation concurrency and action-count assertions in `challenges.integration.test.ts`                              |
| Challenge reads fail closed on missing, extra, or tampered ledger/response bindings and public tombstones exclude retained bytes and private identities | Verified   | Challenge integrity/tamper matrix and serialized public-projection assertions in `challenges.integration.test.ts`                                              |
